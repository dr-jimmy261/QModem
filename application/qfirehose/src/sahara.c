...
uint16_t le_uint16(uint16_t v16) {
...
uint64_t le_uint64(uint64_t v64) {
    const int is_bigendian = 1;
    uint64_t tmp = v64;
    if ((*(char*)&is_bigendian) == 0) {
...
static int send_reset_command(void *usb_handle, void *tx_buffer)
{
    ...
    sahara_reset->q_header.q_len = le_uint32(sizeof(sahara_reset->q_sahara_reset_packet) + sizeof(q_sahara_packet_h));

    /* Send the Reset Request */
    dbg(LOG_EVENT, "SAHARA_RESET ===>");
    if (0 == sahara_tx_data(usb_handle, tx_buffer, sizeof(sahara_reset->q_sahara_reset_packet) + sizeof(q_sahara_packet_h))) {
...
static int send_done_packet(void *usb_handle, void *tx_buffer) {
    ...
    sahara_done->q_header.q_len = le_uint32(sizeof(sahara_done->q_sahara_done_packet) + sizeof(q_sahara_packet_h));
    // Send the image data
    dbg(LOG_EVENT, "Q_SAHARA_FIVE ===>");
    if (0 == sahara_tx_data(usb_handle, tx_buffer, sizeof(sahara_done->q_sahara_done_packet) + sizeof(q_sahara_packet_h)))
    {
...
static int start_image_transfer(void *usb_handle, void *tx_buffer, const struct sahara_pkt *pr_sahara_pkt, FILE *file_handle)
{
    int retval = 0;
...
static int send_hello_response(void *usb_handle, void *tx_buffer, const struct sahara_pkt *sahara_hello)
{
    ...
    sahara_hello_resp->q_header.q_len = le_uint32(sizeof(sahara_hello_resp->q_sahara_hello_packet_response) + sizeof(q_sahara_packet_h));
    sahara_hello_resp->q_sahara_hello_packet_response.q_ver = sahara_hello->q_sahara_hello_packet.q_ver;
    ...
    sahara_hello_resp->q_sahara_hello_packet_response.q_mode = sahara_hello->q_sahara_hello_packet.q_mode;
    ...
    if (0 == sahara_tx_data(usb_handle, tx_buffer, sizeof(sahara_hello_resp->q_sahara_hello_packet_response) + sizeof(q_sahara_packet_h)))
    {
...
int is_multiple_programmer(const char *dir, const char *filename)
{
    DIR *d;
    struct dirent *entry;
...
char* find_image(int id, const char *dir)
{
    char filepath[512];
    FILE *fp;
    char line[1024];
    ...
    snprintf(filepath, sizeof(filepath), "%s/qsahara_device_programmer.xml", dir);
    ...
    {
        if (strstr(line, "<image") != NULL && strstr(line, "image_id") != NULL)
        {
            int image_id = -1;
            char image_path[256];

            char *id_ptr = strstr(line, "image_id=\"");
            char *path_ptr = strstr(line, "image_path=\"");

            if (id_ptr != NULL && path_ptr != NULL) {
                id_ptr += 10;
                image_id = atoi(id_ptr);

                path_ptr += 12;
                char *end_quote = strchr(path_ptr, '"');
                if (end_quote != NULL) {
                    int len = end_quote - path_ptr;
                    strncpy(image_path, path_ptr, len);
                    image_path[len] = '\0';
...
static int sahara_flash_all(void *usb_handle, void *tx_buffer, void *rx_buffer, FILE *file_handle, const char *firehose_dir)
{
...
__wait_hello_packet:
    ...
    {
        dbg(LOG_ERROR, "Received a different q_cmd: %x while waiting for hello packet", pr_sahara_pkt->q_header.q_cmd);
    ...
    while (1)
    {
        ...
        {
            if (q_image_id == 0) q_image_id = le_uint32(pr_sahara_pkt->q_sahara_read_packet_data_64bit.q_image_id);

            if (cur_image == NULL && is_multiple_programmer(firehose_dir, "qsahara_device_programmer.xml"))
            {
                cur_image = find_image(q_image_id, firehose_dir);
                file_handle = unzip_fopen(cur_image, "rb");
            }
        ...
        {
            dbg(LOG_EVENT, "q_image_id = %d, q_status = %d", le_uint32(pr_sahara_pkt->q_sahara_end_packet_image_tx.q_image_id), le_uint32(pr_sahara_pkt->q_sahara_end_packet_image_tx.q_status));
            if (le_uint32(pr_sahara_pkt->q_sahara_end_packet_image_tx.q_status) == Q_SAHARA_STATUS_ZERO)
            {
        ...
        {
            dbg(LOG_ERROR, "Received an unknown q_cmd: %d ", le_uint32(pr_sahara_pkt->q_header.q_cmd));
    ...
    if (Q_SAHARA_MODE_ZERO == le_uint32(pr_sahara_pkt->q_sahara_done_packet_response.q_image_tx_status))
    {
        if (is_multiple_programmer(firehose_dir, "qsahara_device_programmer.xml"))
        {
            q_image_id = 0;
            cur_image = NULL;
            goto __wait_hello_packet;
        }
    ...
    else if (Q_SAHARA_MODE_ONE == le_uint32(pr_sahara_pkt->q_sahara_done_packet_response.q_image_tx_status))
    {
...
int sahara_main(const char *firehose_dir, const char *firehose_mbn, void *usb_handle, int edl_mode_05c69008) {
    ...
    file_handle = unzip_fopen(full_path, "rb");
    if (file_handle == NULL)
    {
    ...
    if (NULL == rx_buffer || NULL == tx_buffer)
    {
        ...
        safe_free(rx_buffer);
        safe_free(tx_buffer);
        unzip_fclose(file_handle);
        file_handle = NULL;
        return ENOMEM;
    }

    retval = is_multiple_programmer(firehose_dir, "qsahara_device_programmer.xml");
    if (retval)
    {
        dbg_time("Multiple programmer maps: %s/qsahara_device_programmer.xml\n", firehose_dir);
        unzip_fclose(file_handle);
        file_handle = NULL;
    }

    retval = sahara_flash_all(usb_handle, tx_buffer, rx_buffer, file_handle, firehose_dir);
    if (0 == retval)
    {
