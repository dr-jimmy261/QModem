...
#define MAX_USBFS_BULK_OUT_SIZE (28 * 1024 + 1)
...
static void* usb_bulk_read_thread(void* arg) {
    ...
    while(usb_handle) {
        int count = qusb_noblock_read(usb_handle,  buf, MAX_USBFS_BULK_IN_SIZE, 1, 120000);
...
int usb2tcp_main(const void *usb_handle, int tcp_port, unsigned qusb_zlp_mode) {
    ...
    int server_fd = -1, client_fd = -1, usb_fd = -1, size = -1, res = -1;
    TLV_USB tlv_usb;
    ...
    while (usb_fd > 0 && client_fd > 0) {
        ...
        for (ne = 0; ne < nevents; ne++) {
            ...
            dprintf("ready to poll and fd == [%d]\r\n", fd == usb_fd ? 1 : 2);
            ...
            if (fd == client_fd) {
                ...
                res = qusb_noblock_write(usb_handle, pbuf, size, size, 3000, qusb_zlp_mode);
                dprintf("from client read size = [%d] and send =[%d]\r\n", size, res);
                // continue;
            }
            else if (fd == usb_fd) {
